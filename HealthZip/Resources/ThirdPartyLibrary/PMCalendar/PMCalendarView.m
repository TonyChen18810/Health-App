...
- (void) setCurrentDate:(NSDate *)currentDate
{
    ...
    NSDateComponents *eComponents = [gregorian components:NSCalendarUnitDay
                                     | NSCalendarUnitMonth
                                     | NSCalendarUnitYear
...
- (UIFont *) font
{
    ...
    {
        _font = [UIFont fontWithName:FONT_ITCAVANTGARDESTD_BK  size: newFontSize];
...
- (void)setPeriod:(PMPeriod *)period
{
    ...
    //Changed by Pankaj--- commented bellow condition to get current date when user tap on current date
...
//    {
        ...
        if (!_currentDate)
        {
          ...
          //Changed by Pankaj -- added bool value to get month is changed or date is selected
        if ([self.delegate respondsToSelector:@selector(periodChanged:isMonthChanged:)])
        {
            [self.delegate periodChanged:_period isMonthChanged:NO];
        }
...
- (NSDate *) dateForPoint: (CGPoint)point
{
    ...
    monthStartDay           = (monthStartDay + (self.mondayFirstDayOfWeek?5:6)) % 7;
    numDaysInMonth         += monthStartDay;
    long maxNumberOfRows     = ceil((CGFloat)numDaysInMonth / 7) - 1;
...
- (void) periodSelectionStarted: (CGPoint) point
{
    ...
    NSDateComponents *dateComponents = [gregorian components:(NSCalendarUnitDay|NSCalendarUnitMonth|NSCalendarUnitYear) fromDate:calcPeriod.startDate];
...
- (void) periodSelectionChanged: (CGPoint) point
{
    ...
    NSCalendar *gregorian = [NSCalendar currentCalendar];
    NSDateComponents *dateComponents = [gregorian components:(NSCalendarUnitDay|NSCalendarUnitMonth|NSCalendarUnitYear) fromDate:newDate];
    NSInteger month = [dateComponents month];
...
- (void) tapHandling: (UIGestureRecognizer *)recognizer
{
    ...
    if(!self.showOnlyCurrentMonth)
    {
        ...
        isMonthChanged =YES;
        //we can know here that user has taped on month chnage button
    }

    //Changed by Pankaj -- added bool value to get month is changed or date is selected
    if ([self.delegate respondsToSelector:@selector(periodChanged:isMonthChanged:)])
    {
        [self.delegate periodChanged:_period isMonthChanged:isMonthChanged];
    }
...
- (void)drawRect:(CGRect)rect
{
	...
	long weekdayOfFirst = ([dateOnFirst weekday] + (_mondayFirstDayOfWeek?5:6)) % 7 + 1;
	long numDaysInMonth = [dateOnFirst numberOfDaysInMonth];
    NSDate *monthStartDate = [_currentDate monthStartDate];
    long todayIndex = [[[NSDate date] dateWithoutTime] daysSinceDate:monthStartDate] + weekdayOfFirst - 1;
    ...
    long selectionStartIndex = [[self.selectedPeriod normalizedPeriod].startDate daysSinceDate:firstDateInCal];
    long selectionEndIndex = [[self.selectedPeriod normalizedPeriod].endDate daysSinceDate:firstDateInCal];
