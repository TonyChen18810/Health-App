...
//    self.initialSize = size;
    ...
    [self.mainView bringSubviewToFront:self.calendarView];
...
//                                              controller:self];
        self.view = [[PMDimmingView alloc] initWithFrame:[[UIScreen mainScreen] bounds] controller:self];
...
- (void) periodChanged:(PMPeriod *)newPeriod isMonthChanged:(BOOL)isChanged
{
    if ([self.delegate respondsToSelector:@selector(calendarController:didChangePeriod:)])
    {
        self.isOnlyMonthChnaged = isChanged;
...
- (void) currentDateChanged:(NSDate *) currentDate
{
	...
	NSUInteger numDaysInMonth      = [currentDate numberOfDaysInMonth];
